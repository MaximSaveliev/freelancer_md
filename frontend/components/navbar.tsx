      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div className="flex items-center justify-between h-20">
            ...
            {isLoggedIn ? (
              ...
              <>
                <Link href="/" className={`text-sm font-semibold relative group ${pathname === '/' ? 'text-white' : 'text-slate-300 hover:text-white'}`}>
                  Как это работает
                  {pathname === '/' && <span className="absolute -bottom-1.5 left-0 w-full h-0.5 bg-primary rounded-full"></span>}
                </Link>
                <Link href="/projects" className={`text-sm font-medium transition-colors ${pathname === '/projects' ? 'text-white' : 'text-slate-300 hover:text-white'}`}>Найти проекты</Link>
          ...
          <div className="hidden md:flex items-center gap-6">
                  ...
                  >
                    <div className="py-1">
                      {languages.map((lang) => (
                        ...
                        >
                          {lang.name}
                          {currentLang === lang.code && <span className="w-1.5 h-1.5 rounded-full bg-primary"></span>}
                        </button>
                      ))}
                    </div>
                  </motion.div>
            ...
            <div className="w-px h-5 bg-slate-border"></div>
            ...
            <div className="flex items-center gap-4">
              {isLoggedIn ? (
                ...
                <>
                  ...
                  <Link 
                    href="/register"
                    className="bg-primary hover:bg-primary/90 text-background-dark text-sm font-bold px-5 py-2.5 rounded-xl transition-all shadow-[0_0_15px_rgba(19,200,236,0.3)] hover:shadow-[0_0_20px_rgba(19,200,236,0.5)] cursor-pointer"
                  >
                    Регистрация
                  </Link>
                </>
